...
export function useConversationLoader({
  ...
  // Lazily created the first time it's needed — NOT during render, so render
  // stays pure (no side effects under concurrent/StrictMode). `getAbortReplacer`
  // is the only writer; both `load` and the unmount cleanup go through it, so the
  // factory runs exactly once and the instance is stable thereafter.
  const fetchAbortRef = useRef<ReturnType<typeof createAbortReplacer> | null>(null);
  const getAbortReplacer = useCallback(() => {
    if (fetchAbortRef.current === null) {
      fetchAbortRef.current = createAbortReplacer();
    }
    return fetchAbortRef.current;
  }, []);
  useEffect(() => {
    isMountedRef.current = true;
    return () => {
      isMountedRef.current = false;
      // Abort an in-flight fetch on unmount. Skip if nothing ever started one
      // (ref still null) — no work to cancel, and we avoid creating a replacer
      // just to immediately abort it.
  ...
  const load = useCallback(
    async (showSpinner: boolean) => {
      ...
      const signal = getAbortReplacer().begin();
