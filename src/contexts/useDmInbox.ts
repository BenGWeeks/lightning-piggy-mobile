...
export interface UseDmInboxResult {
  ...
  // `getCachedConversation` is deliberately NOT exposed: it's an internal input
  // to this read-through, so callers can't bypass the union and paint a
  // cache-only thread that lags the inbox.
...
export function useDmInbox(options: UseDmInboxOptions): UseDmInboxResult {
  ...
  // Internal-only (#869): a private input to loadInitialConversation's
  // read-through union, NOT part of UseDmInboxResult — see the interface note.
