...
- New: Send to a Bitcoin address from a Lightning wallet — the Boltz reverse-swap path now shows the Boltz logo next to the swap-fee line, surfaces the underlying error if a claim fails, and (most importantly) persists every swap so a force-stop or network hiccup mid-flight can be recovered via "Retry now" or the next app launch. Previously a failed claim left funds stuck in the Boltz HTLC.
...
- Perf: cold-start "tap Send sheet feels frozen for ~12 s" path tackled in multiple layers — yield more often in inbox drain, dedupe the SDK's per-wrap NIP-04 deprecation warning, 1.5 s grace window before bulk relay/profile/contact refresh fires, yield the JS event loop between each live-sub wrap decrypt, eager-hydrate own profile + relays from cache so the drawer paints from frame 1. Console.log calls are now stripped from production bundles too.
