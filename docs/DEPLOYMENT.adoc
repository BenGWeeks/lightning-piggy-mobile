...
#
# TMPDIR=~/eas-tmp is REQUIRED on hosts with /tmp on tmpfs (default
# Ubuntu/Debian setup, 12 GB cap). The EAS local builder generates
# ~9 GB of native-lib intermediates during the multi-arch APK assembly
# (arm64 + armv7 + x86 + x86_64), and Gradle crashes with
...
# gives Gradle the ~10 GB of headroom it needs.
mkdir -p ~/eas-tmp
TMPDIR=~/eas-tmp EXPO_NO_DOCTOR=1 eas build --local --platform android --profile production --non-interactive
...
`TMPDIR=~/eas-tmp` knob above is what you're missing. The `~/eas-tmp`
working tree can be deleted between builds (`rm -rf ~/eas-tmp`) to
reclaim disk; the next `mkdir -p ~/eas-tmp` re-creates it before the
build.
