...
WARNING: **This is not the release path.** Cutting a release goes through `.github/workflows/release.yml` (see "Cutting a release" below), which runs EAS cloud builds anchored to the published tag. Running `eas build --local` manually for a release detaches the artifact from any commit — anything merged to `main` during the ~30 min build window quietly ships too, and the GitHub Release notes drift away from what's actually deployed. We hit exactly this on v1.1.0 (two PRs landed on main between the version bump and the local build finishing, neither belonged to the release).

The fallbacks below are for two narrow cases:

. **Sideloading a release-flavor APK to a device for testing** — usually for R8 / proguard validation. Prefer pulling the latest cloud-built APK instead: `eas build:download --platform android --latest` followed by `adb install -r`. Only fall back to a local build when you're offline or the EAS queue is jammed.
...
**Always go through this workflow — never run `eas build` manually for a release.** The workflow tags first then builds, so artifacts are anchored to one commit; manual builds let anything merged to `main` during the build window quietly ship too (v1.1.0 hit this — two unrelated PRs landed mid-build).
...
. Once the iOS build appears in App Store Connect, paste the "What to Test" notes manually — EAS's `--what-to-test` flag is Enterprise-tier only.
